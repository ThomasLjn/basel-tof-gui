...
class Signal:
    def __init__(self, fname):
        self.fname = fname
        self.impath = None #path to the image relative to the crystal
        self.vrods = None
        self.vmesh = None
        self.x = None
        self.y = None
        self.y1 = None #first derivative
        self.ca_int = []
        self.bl_int = []
        self.bl_dx = 80000
        self.bl_center = -25000
        self.y_smooth = None
        self.peak_ind = [] #index of tof
        self.peak_time = [] #tof
        self.peak_amp = [] #amplitude
        self.peak_start = [] #index
        self.peak_end = [] #index
...
class Simulation:
    ...
    def get_data(self,skiprow=2):
        bins = np.arange(0,25,0.005)
        n_filt = 2
        w_filt = gaussian(n_filt,0.25*n_filt)
        w_filt = w_filt / w_filt.sum()
        _, count, mass, _, _, tof = np.loadtxt(self.fname, delimiter = ',', unpack = True,skiprows=skiprow)
        hist_all, x_edges = np.histogram(tof, bins = bins)
        x_0 = x_edges[:-1] + 0.5 * np.diff(x_edges)
        y_0 = np.convolve(hist_all, w_filt, mode = 'same')
        self.x = x_0
        self.y = y_0
...
class App(tk.Tk):
    ...
    def create_left_frame(self, left_frame):
        ...
        self.file_cbox['values'] = ['Experiment', 'Simulation']
        ...
        self.auto_offset_btn = ttk.Button(left_frame, text='Auto Offset', command=self.auto_offset)
        self.manual_offset_label = ttk.Label(left_frame, text="Manual offset")
        self.manual_offset_entry = ttk.Entry(left_frame)
        self.manual_offset_btn = ttk.Button(left_frame, text="Manual offset", command=self.manual_offset)
        ...
        # self.auto_offset_btn.grid(row=5, columnspan=2)
    ...
    def create_plot_frame(self, plot_frame):
        ...
        self.main_ax.set_xlabel('TOF (micros)')
        self.main_ax.set_ylabel('Signal (V)')
        self.toolbar_frame = tk.Frame(self.plot_frame)
        self.toolbar = NavigationToolbar2Tk(self.main_figure_canvas, self.toolbar_frame) 
        self.main_figure_canvas.get_tk_widget().pack()
        self.toolbar_frame.pack() 

        self.main_line_sim = None
        self.main_line = None
    ...
    def select_file(self):
        ...
        elif self.file_cbox.get() == 'Simulation':
            filetypes = (('All files', '*.*'))
            filename = fd.askopenfilename(
                title='Open a file',
                initialdir='./')
            self.sim = Simulation(filename)
            self.sim.get_data()
            if self.s != None:
                self.plot_sim_data(self.sim, self.main_ax, self.main_figure_canvas, s=self.s)
            else:
                self.plot_sim_data(self.sim, self.main_ax, self.main_figure_canvas)
    ...
    def plot_sim_data(self, sim, ax, fig_canvas, offset=0, s=None):
        # self.sim_line = ax.plot(sim.x+offset, sim.y)
        if s != None:
            Y = sim.y/np.max(sim.y)
            Y *= np.min(s.y)
            self.auto_offset_btn.pack()
            self.manual_offset_label.pack()
            self.manual_offset_entry.pack()
            self.manual_offset_btn.pack()
        else:
            Y = sim.y
        self.main_line_sim = ax.plot(sim.x+offset, Y, color='orange')
        fig_canvas.draw()
    ...
    def plot_exp_data(self, s, ax, fig_canvas):
        self.main_line = ax.plot(s.x, s.y, color='C0')
    ...
    def auto_offset(self):
        ...
        sim = self.sim
        try:
            #offset = Xexp[np.argmin(Yexp)] - Xsim[np.argmin(Ysim_scaled)]
            offset = s.x[np.argmin(s.y)] - sim.x[np.argmax(sim.y)]
            self.clear_sim()
            self.plot_sim_data(sim, self.main_ax, self.main_figure_canvas, offset, s)
            # self.clear_sim()
    ...
    def manual_offset(self):
        ...
        try:
            #offset = Xexp[np.argmin(Yexp)] - Xsim[np.argmin(Ysim_scaled)]
            offset = float(self.manual_offset_entry.get())
            self.clear_sim()
            self.plot_sim_data(sim, self.main_ax, self.main_figure_canvas, offset, s)
            self.main_figure_canvas.draw()
        except:
